...
import { path, createElementTree, debugLog, lockInputMethodComposition } from "./utils"
...
export class ImageBatchRenameModal extends Modal {
  ...
  constructor(app: App, activeFile: TFile | undefined, renameFunc: renameFuncType, onClose: () => void) {
    super(app)
  ...
  onOpen() {
    ...
    const namePatternSetting = new Setting(contentEl)
      ...
      .addText((text) =>
        text.setValue(this.state.namePattern).onChange(async (value) => {
    ...
    const nameReplaceSetting = new Setting(contentEl)
      ...
      .addText((text) =>
        text.setValue(this.state.nameReplace).onChange(async (value) => {
  ...
  matchImageNames(tbodyEl: HTMLElement) {
    ...
    tbodyEl.empty()
    const fileCache = this.activeFile ? this.app.metadataCache.getFileCache(this.activeFile) : null
